...
class ThreadTaskGetBusIdByUsername (private val activity: UpdateViewInterface) : Thread() {
        private var result = "NOT SET YET"
        private var postData: HashMap<String, String>? = null



        fun setPostData(username: String) {
            postData = hashMapOf(
        ...
        override fun run() {
            ...
            Log.d("ThreadTaskGetIdbyUser", "Starting thread task")

            try {
                val url = URL("http://499aitikira.cs.umd.edu/cgi-bin/findBusbyUsername.php")
                ...
                Log.d("ThreadTaskFindBusbyUser", "Response from server: $result")

            } catch (e: IOException) {
                Log.e("ThreadTaskFindBusbyUser", "IOException: ${e.message}")
                result = "IOException: ${e.message}"
            } catch (e: Exception) {
                Log.e("ThreadTaskFindBusbyUser", "Exception: ${e.message}")
