...
class ThreadTaskShowOrders (private val activity: UpdateViewInterface) : Thread() {
    ...
    override fun run() {
        ...
        Log.d("ThreadTaskShowOrders", "Starting thread task")

        try {
            val url = URL("http://499aitikira.cs.umd.edu/cgi-bin/showOrders.php")
            connection = url.openConnection() as HttpURLConnection
            connection.requestMethod = "POST"
            connection.doOutput = true
            connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded")
            Log.d("ThreadTaskShowOrders", "Sending POST data: ${getPostDataString(postData)}")
            ...
            Log.d("ThreadTaskShowOrders", "Response from server: $result")

        } catch (e: IOException) {
            Log.e("ThreadTaskShowOrders", "IOException: ${e.message}")
            result = "IOException: ${e.message}"
        } catch (e: Exception) {
            Log.e("ThreadTaskShowOrders", "Exception: ${e.message}")
